...
class Level51: SKScene {
    ...
    var jumpPad2 = SKShapeNode(rectOf: CGSize(width: 100, height: 15))
    var spikes: SKSpriteNode!
    var spikes2: SKSpriteNode!
    ...
    override func didMove(to view: SKView) {
        ...
        player.position = CGPoint(x: 483, y: -94)
        ...
        jumpPad.position = CGPoint(x: 285, y: -290)
        addChild(jumpPad)
        
        jumpPad2.fillColor = (SKColor(red: 0.2196, green: 0.2196, blue: 0.2196, alpha: 1.0))
        jumpPad2.strokeColor = (SKColor(red: 0, green: 0, blue: 0, alpha: 1.0))
        jumpPad2.position = CGPoint(x: 205, y: -290)
        addChild(jumpPad2)
        
        spikes = SKSpriteNode(imageNamed: "Spikes")
        spikes.position = CGPoint(x: -400, y: -200)
        addChild(spikes)

        spikes2 = SKSpriteNode(imageNamed: "Spikes")
        spikes2.position = CGPoint(x: -400, y: -290)
        spikes.zRotation = CGFloat.pi
        addChild(spikes2)
        ...
        createShape(x: 512, y: -134, width: 129, height: 25)
    ...
    override func keyDown(with event: NSEvent) {
        ...
        } else if event.keyCode == 49 && player.intersects(jumpPad2) { // Space bar keycode
            player.physicsBody?.applyImpulse(CGVector(dx: 0, dy: 75))
...
//        }
        ...
        if player.position.x < -520 {
            let level52 = SKScene(fileNamed: "Level52")
            level52?.scaleMode = .aspectFill
            scene?.view?.presentScene(level52)
        }
